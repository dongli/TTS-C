program CalcExactMovingVortices

    use MsgManager
    use RunManager
    use SphereService
    use MeshManager
    use MovingVorticesTestbed
    use NFWrap
    ...
    call MsgManager_RecordSpeaker("CalcExactMovingVortices")
    ...
    call NFWrap_NewScalar(fcard, "vtxlon", "vortex center longitude", "degree_E")
    call NFWrap_NewScalar(fcard, "vtxlat", "vortex center latitude", "degree_N")
    allocate(qExact(m%numLon,0:m%numLat+1))

    do k = 0, numTimeStep
        call NFWrap_Advance(fcard, k, k*dt)
        call MovingVorticesTestbed_Advance(k*dt)
        call NFWrap_OutputScalar(fcard, "vtxlon", lonVortex*Rad2Deg)
        call NFWrap_OutputScalar(fcard, "vtxlat", latVortex*Rad2Deg)
...
end program CalcExactMovingVortices
